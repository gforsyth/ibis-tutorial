...
Your coworker is a SQL expert, but they're on vacation -- they gave you this query that returns the most recursively depended on packages, but it's using a recursive Common Table Expression and you only really feel safe editing the `LIMIT` statement.

You need to find out if any packages maintained by NVIDIA are in the top 100 most-depended-upon-packages -- luckily we know that NVIDIA really likes to add the string `"nvidia"` to the names of their packages...
...
SELECT
  dependency,
  count(DISTINCT package) AS n_dependents
FROM transitive_deps
GROUP BY
  dependency
ORDER BY
